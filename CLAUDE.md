...
was deleted, and the 2026-08-23/24 `detail` passes filled most of the rest in, so 14 of 748 items today are
name-only), and an item whose `term` runs past 34 characters is laid out as a full-width line instead of a
name-column entry (110 items — they are shell one-liners, not names). Both add `.item-line`, which is **excluded from the card's subgrid**; if it
...
`search-index.js` (442 KB raw / 129 KB gzip — it grew with the 2026-08-23/24 `detail` passes; each item
